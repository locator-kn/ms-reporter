...
fns.recommendationForPerson = (request, next) => {
    ...
    ger.recommendations_for_person(data.namespace ||'locations', data.user_id || 'timo', {actions: data.actions || {view: 1, likes: 1}})
...
fns.recommendationForThing = (request, next) => {
    let data = request.data || {};
    ger.recommendations_for_thing(data.namespace ||'locations', data.thing_id || '56915a73a5963dcf23aeeedb', {actions: data.actions || {view: 1, likes: 1}})
        .then(res => next(null, res))
        .catch(next);
};
