...
fns.addimpression = (message) => {
    console.log('uff, training', message);
    trainer.train('locations', message.requesting_user_id, 'addimpression', message.data.location_id, getExpireDate(4));
};


fns.toggleFavor = (message) => {
    console.log('uff, training', message);
    trainer.train('locations', message.requesting_user_id, 'likes', message.data.location_id, getExpireDate(4));
};
